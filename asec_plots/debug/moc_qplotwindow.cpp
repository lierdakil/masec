/****************************************************************************
** Meta object code from reading C++ file 'qplotwindow.h'
**
** Created: Thu 20. Nov 00:09:20 2008
**      by: The Qt Meta Object Compiler version 59 (Qt 4.4.3)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../qplotwindow.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'qplotwindow.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 59
#error "This file was generated using the moc from 4.4.3. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_QPlotWindow[] = {

 // content:
       1,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   10, // methods
       0,    0, // properties
       0,    0, // enums/sets

 // slots: signature, parameters, type, tag, flags
      18,   13,   12,   12, 0x0a,
      37,   13,   12,   12, 0x0a,
      61,   12,   56,   12, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_QPlotWindow[] = {
    "QPlotWindow\0\0data\0new_X(QStringList)\0"
    "new_Y(QStringList)\0bool\0editparams()\0"
};

const QMetaObject QPlotWindow::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_QPlotWindow,
      qt_meta_data_QPlotWindow, 0 }
};

const QMetaObject *QPlotWindow::metaObject() const
{
    return &staticMetaObject;
}

void *QPlotWindow::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_QPlotWindow))
        return static_cast<void*>(const_cast< QPlotWindow*>(this));
    return QWidget::qt_metacast(_clname);
}

int QPlotWindow::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: new_X((*reinterpret_cast< QStringList(*)>(_a[1]))); break;
        case 1: new_Y((*reinterpret_cast< QStringList(*)>(_a[1]))); break;
        case 2: { bool _r = editparams();
            if (_a[0]) *reinterpret_cast< bool*>(_a[0]) = _r; }  break;
        }
        _id -= 3;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
